...
public sealed class RollbackImportacaoService(ApplicationDbContext db, ILogger<RollbackImportacaoService> logger) : IRollbackImportacaoService
{
    ...
    public async Task<ResultadoRollbackImportacao> ExecutarAsync(Guid id, Guid company, Guid? user, bool partial, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    var h = await db.ImportacoesHistorico.FirstAsync(x => x.Id == id && x.CompanyId == company, ct); var executionEnd = h.FinalizadoEm ?? h.CreatedAt; h.IniciarRollback(user); await db.SaveChangesAsync(ct); var items = await db.ImportacaoItens.Where(x => x.CompanyId == company && x.ImportacaoHistoricoId == id && x.ProdutoId.HasValue && (x.Status == StatusLinhaImportacao.Inserida || x.Status == StatusLinhaImportacao.Atualizada)).OrderBy(x => x.NumeroLinha).ToListAsync(ct); var ids = items.Select(x => x.ProdutoId!.Value).Distinct().ToList(); var products = await db.Products.IgnoreQueryFilters().Where(x => x.CompanyId == company && ids.Contains(x.Id)).ToDictionaryAsync(x => x.Id, ct); var origin = $"importação:{id}"; var later = (await db.ProductChangeHistories.AsNoTracking().Where(x => x.CompanyId == company && ids.Contains(x.ProductId) && x.CreatedAt > executionEnd && x.Origin != origin && x.Origin != $"rollback:{id}").Select(x => x.ProductId).Distinct().ToListAsync(ct)).ToHashSet(); var removed = 0; var restored = 0; var blocked = 0; var failures = 0; var results = new List<ResultadoRollbackItem>();
                    foreach (var item in items) { ct.ThrowIfCancellationRequested(); var pid = item.ProdutoId!.Value; if (later.Contains(pid)) { item.RegistrarRollback(false, "Bloqueado por alterações posteriores."); blocked++; results.Add(new(item.NumeroLinha, pid, "Bloquear", false, item.MensagemRollback!)); continue; } if (!products.TryGetValue(pid, out var p)) { item.RegistrarRollback(false, "Produto não encontrado."); failures++; results.Add(new(item.NumeroLinha, pid, "Falha", false, item.MensagemRollback!)); if (!partial) throw new RollbackImportacaoException(item.MensagemRollback!); continue; } try { if (item.Status == StatusLinhaImportacao.Inserida) { p.DeletedBy = user; db.Products.Remove(p); removed++; item.RegistrarRollback(true, "Produto removido logicamente."); db.ProductChangeHistories.Add(new(company, p.Id, "Produto", "Ativo", "Excluído", $"rollback:{id}") { CreatedBy = user }); results.Add(new(item.NumeroLinha, pid, "Remover", true, item.MensagemRollback!)); } else { Restore(p, item.AlteracoesAplicadasJson); p.UpdatedBy = user; restored++; item.RegistrarRollback(true, "Valores anteriores restaurados."); db.ProductChangeHistories.Add(new(company, p.Id, "Produto", "Importado", "Restaurado", $"rollback:{id}") { CreatedBy = user }); results.Add(new(item.NumeroLinha, pid, "Restaurar", true, item.MensagemRollback!)); } } catch (Exception ex) when (ex is ArgumentException or InvalidOperationException or JsonException) { logger.LogWarning(ex, "Rollback bloqueado para produto {Product}", pid); item.RegistrarRollback(false, ex.Message); failures++; results.Add(new(item.NumeroLinha, pid, "Falha", false, ex.Message)); if (!partial) throw; } }
    ...
    private static void Restore(Product p, string? json) { if (string.IsNullOrWhiteSpace(json)) throw new RollbackImportacaoException("Snapshot de alterações indisponível."); var changes = JsonSerializer.Deserialize<Dictionary<string, RollbackChange>>(json) ?? throw new JsonException(); string? Old(string key, string? fallback) => changes.TryGetValue(key, out var x) ? x.ValueString() : fallback; var unit = Guid.TryParse(Old("Unidade", p.UnitOfMeasureId.ToString()), out var u) ? u : p.UnitOfMeasureId; var type = Enum.TryParse<ProductType>(Old("Tipo", p.ProductType.ToString()), out var t) ? t : p.ProductType; var cost = ParseDecimal(Old("Preço de custo", p.CostPrice.ToString()), p.CostPrice); var sale = ParseDecimal(Old("Preço de venda", p.SalePrice.ToString()), p.SalePrice); p.Update(p.InternalCode, p.Sku, Old("Código de barras", p.Barcode), p.Reference, Old("Descrição", p.Name)!, p.ShortDescription, p.Description, type, p.ControlsStock, p.IsActive, p.CategoryId, p.SubcategoryId, p.BrandId, unit, p.ProductGroupId, p.MainSupplierId, p.PartnerId, p.DefaultWarehouseId, p.DefaultWarehouseLocationId, p.Ncm, p.Cest, cost, sale, p.CommissionType, p.CommissionValue, p.PriceValidUntil, p.MinimumStock, p.Notes); }
