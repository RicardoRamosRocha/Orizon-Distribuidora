...
public sealed class ModeloImportacaoService(ApplicationDbContext db) : IModeloImportacaoService
{
    ...
    {
        var validacao = ValidadorMapeamentoColunas.Validar(mapeamentos, cabecalhos);
        if (!validacao.Valido) throw new ArgumentException(string.Join(" ", validacao.Erros));
        if (padrao) foreach (var item in await db.ModelosImportacao.Where(x => x.CompanyId == empresaId && x.UsuarioId == usuarioId && x.Padrao).ToListAsync(cancellationToken)) item.DefinirComoPadrao(false);
    ...
    public async Task ExcluirAsync(Guid empresaId, Guid id, Guid? usuarioId, CancellationToken cancellationToken = default)
    {
        var model = await db.ModelosImportacao.FirstOrDefaultAsync(x => x.CompanyId == empresaId && x.Id == id, cancellationToken) ?? throw new KeyNotFoundException();
        if (model.UsuarioId.HasValue && model.UsuarioId != usuarioId)
            throw new UnauthorizedAccessException("O modelo privado pertence a outro usuário.");
        model.DeletedBy = usuarioId;
        db.ModelosImportacao.Remove(model);
